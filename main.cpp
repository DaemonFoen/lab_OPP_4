...
double CalculateDelta(double *Omega) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
            ...
            for (int k = 0; k < N; k++) {
                z = Z(k);
                deltaMax = std::max(deltaMax, std::abs(Omega[idx(i, j, k)] - phi(x, y, z)));
            }
        }
    }
    return deltaMax;
...
int main(int argc, char *argv[]) {
    ...
    do {
        ...
        if (ProcRank != 0) {
            MPI_Isend(CurrentLayerBuf + N * N, N * N, MPI_DOUBLE, ProcRank - 1, TAG, MPI_COMM_WORLD, &req[1]);
            MPI_Irecv(CurrentLayerBuf, N * N, MPI_DOUBLE, ProcRank - 1, TAG, MPI_COMM_WORLD, &req[0]);
        }

        if (ProcRank != ProcNum - 1) {
            MPI_Isend(CurrentLayerBuf + N * N * LayerSize, N * N, MPI_DOUBLE, ProcRank + 1, TAG, MPI_COMM_WORLD, &req[3]);
            MPI_Irecv(CurrentLayerBuf + N * N * (LayerSize + 1), N * N, MPI_DOUBLE, ProcRank + 1, TAG, MPI_COMM_WORLD, &req[2]);
        }
    ...
    MPI_Gather(CurrentLayer + N * N, LayerSize * N * N, MPI_DOUBLE, Omega, LayerSize * N * N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
